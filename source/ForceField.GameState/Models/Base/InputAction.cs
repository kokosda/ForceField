...
namespace ForceField.GameState.Models.Base
{
    ...
    public class InputAction
    {
        ...
        private readonly MouseScroll[] mouseScrolls;
        ///<summary>
        ...
        private delegate bool MouseScrollPressed(MouseScroll button);
        ...
        public InputAction(Buttons[] buttons, Keys[] keys, MouseScroll[] mouseScrolls, bool newPressOnly)
        {
            ...
            this.mouseScrolls = mouseScrolls != null ? mouseScrolls.Clone() as MouseScroll[] : new MouseScroll[0];
        ...
        public bool Evaluate(InputService state, PlayerIndex? controllingPlayer, out PlayerIndex player)
        {
            // Figure out which delegate methods to map from the state which takes care of our "newPressOnly" logic
            ButtonPress buttonTest;
            KeyPress keyTest;
            MouseScrollPressed scrollTest;
            if (newPressOnly)
            {
                buttonTest = state.IsButtonPress;
                keyTest = state.IsKeyPress;
            }
            else
            {
            ...
            scrollTest = state.IsMouseScrollPressed;
            ...
            foreach (MouseScroll mouseScroll in mouseScrolls)
            {
                if (scrollTest(mouseScroll))
                {
